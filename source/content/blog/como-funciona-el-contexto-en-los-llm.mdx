...
humanReviewed: true
heroImage: ../../assets/post/contexto-llm-hero.png
---
...
Si quieres verlo en acción, en el [tokenizador interactivo del laboratorio](/laboratorio/tokenizador/) puedes escribir cualquier texto y observar en tiempo real cómo se trocea en tokens.
...
Esa gestión del contexto es, de hecho, la base técnica de una disciplina más amplia: [el loop engineering](/prompt-engineering-vs-loop-engineering/) diseña sistemas completos de acción-observación-corrección alrededor de esta misma ventana limitada, en vez de optimizar un único prompt.

¿Quieres ponerle números? La [calculadora de contexto y costes del laboratorio](/laboratorio/calculadora-contexto/) compara cuánto ocupa y cuesta tu texto en los principales modelos actuales.

Para ver cómo un LLM elige el siguiente token a partir de esa distribución de probabilidad —y qué efecto tienen temperatura, top-k y top-p en esa elección—, hay una demostración interactiva en el [simulador de temperatura y sampling del laboratorio](/laboratorio/sampling/).
